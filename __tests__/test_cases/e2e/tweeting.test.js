...
describe('Given an authenticated user', () => {
    ...
    afterAll(async () => {
        // remove user
        await then.delete_user_from_user_pool(user.username);
    });
    ...
    describe('When he sends a tweet', () => {
        ...
        describe('When he calls getTweets', () => {
            ...
            beforeAll(async () => {
                ({ tweets, nextToken } = await when.a_user_calls_getTweets(user, user.username, 25));
            });

            it('He will see his new tweet when he sends a tweet', () => {
            ...
            it('He cannot ask for more than 25 tweets in a page', async () => {
                expect(() => when.a_user_calls_getTweets(user, user.username, 26)).rejects.toMatchObject({
        ...
        describe('When he calls getMyTimeline', () => {
          let tweets, nextToken;
          beforeAll(async () => {
              ({ tweets, nextToken } = await when.a_user_calls_getMyTimeline(user, 25));
          });
          ...
          it('He cannot ask for more than 25 tweets in a page', async () => {
              expect(() => when.a_user_calls_getTweets(user, 26)).rejects.toMatchObject({
                  message: expect.stringContaining('max limit is 25'),
              });
          });
      });
